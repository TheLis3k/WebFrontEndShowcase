...
export default function Loader() {
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => { progress.style.width = '100%' }, 100)
    let t3: ReturnType<typeof setTimeout>
    const t2 = setTimeout(() => {
      loader.style.opacity = '0'
      t3 = setTimeout(() => { loader.style.display = 'none' }, 800)
    }, 1600)
  ...
  return (
    <div ref={loaderRef} id="loader" className="fixed inset-0 z-[10001] bg-lisek-dark flex flex-col items-center justify-center">
      ...
      <div className="w-48 h-1 bg-lisek-brown mt-4 rounded-full overflow-hidden">
        <div ref={progressRef} className="h-full bg-lisek-orange w-0 transition-all duration-[1500ms] ease-out" />
      </div>
    </div>
  )
}
